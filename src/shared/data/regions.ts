export interface Region {
  id: string;
  name: string;
  englishName?: string;
  country: string;
  countryCode: string;
  searchKeywords: string[];
  flag: string;
}

export interface Country {
  id: string;
  name: string;
  englishName: string;
  code: string;
  regions: string[];
  flag: string;
}

export const countries: Country[] = [
  {
    id: "kr",
    name: "대한민국",
    englishName: "South Korea",
    code: "KR",
    flag: "🇰🇷",
    regions: [
      "kr-seoul",
      "kr-busan",
      "kr-jeju",
      "kr-gyeongju",
      "kr-jeonju",
      "kr-gangneung",
      "kr-yeosu",
      "kr-daegu",
      "kr-incheon",
      "kr-chuncheon",
      "kr-andong",
      "kr-tongyeong",
      "kr-pohang",
      "kr-ulsan",
      "kr-daejeon",
      "kr-gwangju",
      "kr-sokcho",
      "kr-gapyeong",
      "kr-namisum",
      "kr-seoraksan",
      "kr-jirisan",
      "kr-hallasan",
      "kr-taean",
      "kr-boryeong",
      "kr-danyang",
      "kr-yeongwol",
      "kr-jeongseon",
      "kr-pyeongchang",
      "kr-goseong",
      "kr-yangyang",
      "kr-suncheon",
      "kr-damyang",
      "kr-hwasun",
      "kr-gokseong",
      "kr-gurye",
    ],
  },
  {
    id: "th",
    name: "태국",
    englishName: "Thailand",
    code: "TH",
    flag: "🇹🇭",
    regions: [
      "th-bangkok",
      "th-pattaya",
      "th-phuket",
      "th-chiangmai",
      "th-ayutthaya",
      "th-kanchanaburi",
      "th-kohsamui",
    ],
  },
  {
    id: "sg",
    name: "싱가포르",
    englishName: "Singapore",
    code: "SG",
    flag: "🇸🇬",
    regions: [],
  },
  {
    id: "mv",
    name: "몰디브",
    englishName: "Maldives",
    code: "MV",
    flag: "🇲🇻",
    regions: [],
  },
  {
    id: "ph",
    name: "필리핀",
    englishName: "Philippines",
    code: "PH",
    flag: "🇵🇭",
    regions: [
      "ph-boracay",
      "ph-cebu",
      "ph-bohol",
      "ph-manila",
      "ph-clark",
      "ph-palawan",
    ],
  },
  {
    id: "vn",
    name: "베트남",
    englishName: "Vietnam",
    code: "VN",
    flag: "🇻🇳",
    regions: [
      "vn-danang",
      "vn-nhatrang",
      "vn-dalat",
      "vn-phuquoc",
      "vn-hanoi",
      "vn-hochiminh",
      "vn-muine",
      "vn-sapa",
      "vn-donghoi",
    ],
  },
  {
    id: "kh",
    name: "캄보디아",
    englishName: "Cambodia",
    code: "KH",
    flag: "🇰🇭",
    regions: ["kh-siemreap", "kh-phnompenh"],
  },
  {
    id: "my",
    name: "말레이시아",
    englishName: "Malaysia",
    code: "MY",
    flag: "🇲🇾",
    regions: ["my-kotakinabalu", "my-kualalumpur"],
  },
  {
    id: "id",
    name: "인도네시아",
    englishName: "Indonesia",
    code: "ID",
    flag: "🇮🇩",
    regions: ["id-bali", "id-batam", "id-manado"],
  },
  {
    id: "la",
    name: "라오스",
    englishName: "Laos",
    code: "LA",
    flag: "🇱🇦",
    regions: ["la-vangvieng", "la-luangprabang", "la-vientiane"],
  },
  {
    id: "tw",
    name: "대만",
    englishName: "Taiwan",
    code: "TW",
    flag: "🇹🇼",
    regions: ["tw-taipei", "tw-taichung", "tw-kaohsiung", "tw-tainan"],
  },
  {
    id: "in",
    name: "인도",
    englishName: "India",
    code: "IN",
    flag: "🇮🇳",
    regions: [],
  },
  {
    id: "np",
    name: "네팔",
    englishName: "Nepal",
    code: "NP",
    flag: "🇳🇵",
    regions: [],
  },
  {
    id: "lk",
    name: "스리랑카",
    englishName: "Sri Lanka",
    code: "LK",
    flag: "🇱🇰",
    regions: [],
  },
  {
    id: "cn",
    name: "중국",
    englishName: "China",
    code: "CN",
    flag: "🇨🇳",
    regions: [
      "cn-zhangjiajie",
      "cn-changbaishan",
      "cn-shanghai",
      "cn-beijing",
      "cn-chengdu",
      "cn-jiuzhaigou",
      "cn-guiyang",
      "cn-anshun",
      "cn-tibet",
      "cn-chongqing",
      "cn-harbin",
      "cn-huangshan",
      "cn-xian",
      "cn-dalian",
      "cn-qingdao",
      "cn-yantai",
      "cn-jinan",
      "cn-weihai",
      "cn-hainan",
    ],
  },
  {
    id: "jp",
    name: "일본",
    englishName: "Japan",
    code: "JP",
    flag: "🇯🇵",
    regions: [
      "jp-tokyo",
      "jp-shizuoka",
      "jp-izu",
      "jp-hakone",
      "jp-fuji",
      "jp-yokohama",
      "jp-kamakura",
      "jp-yamanashi",
      "jp-osaka",
      "jp-kyoto",
      "jp-kobe",
      "jp-wakayama",
      "jp-sarayama",
      "jp-fukuoka",
      "jp-beppu",
      "jp-yufuin",
      "jp-saga",
      "jp-oita",
      "jp-kumamoto",
      "jp-nagasaki",
      "jp-kagoshima",
      "jp-miyazaki",
      "jp-kitakyushu",
      "jp-yamaguchi",
      "jp-sapporo",
      "jp-furano",
      "jp-biei",
      "jp-asahikawa",
      "jp-noboribetsu",
      "jp-hakodate",
      "jp-takamatsu",
      "jp-naoshima",
      "jp-matsuyama",
      "jp-tokushima",
      "jp-tottori",
      "jp-yonago",
      "jp-hiroshima",
      "jp-okayama",
      "jp-okinawa",
      "jp-ishigaki",
      "jp-toyama",
      "jp-alpenroute",
      "jp-nagoya",
      "jp-takayama",
      "jp-komatsu",
      "jp-niigata",
      "jp-aomori",
      "jp-akita",
      "jp-sendai",
      "jp-yamagata",
    ],
  },
  {
    id: "west-eu",
    name: "서유럽",
    englishName: "Western Europe",
    code: "WEU",
    flag: "🇪🇺",
    regions: [
      "weu-uk",
      "weu-switzerland",
      "weu-italy",
      "weu-france",
      "weu-netherlands",
      "weu-belgium",
      "weu-romania",
      "weu-germany",
      "weu-ireland",
    ],
  },
  {
    id: "east-eu",
    name: "동유럽",
    englishName: "Eastern Europe",
    code: "EEU",
    flag: "🇪🇺",
    regions: ["eeu-austria", "eeu-czech", "eeu-hungary"],
  },
  {
    id: "hr",
    name: "크로아티아",
    englishName: "Croatia",
    code: "HR",
    flag: "🇭🇷",
    regions: [],
  },
  {
    id: "si",
    name: "슬로베니아",
    englishName: "Slovenia",
    code: "SI",
    flag: "🇸🇮",
    regions: [],
  },
  {
    id: "es",
    name: "스페인",
    englishName: "Spain",
    code: "ES",
    flag: "🇪🇸",
    regions: [
      "es-barcelona",
      "es-madrid",
      "es-sevilla",
      "es-granada",
      "es-valencia",
      "es-mallorca",
    ],
  },
  {
    id: "pt",
    name: "포르투갈",
    englishName: "Portugal",
    code: "PT",
    flag: "🇵🇹",
    regions: ["pt-lisbon", "pt-porto", "pt-faro"],
  },
  {
    id: "ma",
    name: "모로코",
    englishName: "Morocco",
    code: "MA",
    flag: "🇲🇦",
    regions: [],
  },
  {
    id: "tr",
    name: "터키",
    englishName: "Turkey",
    code: "TR",
    flag: "🇹🇷",
    regions: [],
  },
  {
    id: "gr",
    name: "그리스",
    englishName: "Greece",
    code: "GR",
    flag: "🇬🇷",
    regions: [],
  },
  {
    id: "eg",
    name: "이집트",
    englishName: "Egypt",
    code: "EG",
    flag: "🇪🇬",
    regions: [],
  },
  {
    id: "ge",
    name: "조지아",
    englishName: "Georgia",
    code: "GE",
    flag: "🇬🇪",
    regions: [],
  },
  {
    id: "az",
    name: "아제르바이잔",
    englishName: "Azerbaijan",
    code: "AZ",
    flag: "🇦🇿",
    regions: [],
  },
  {
    id: "am",
    name: "아르메니아",
    englishName: "Armenia",
    code: "AM",
    flag: "🇦🇲",
    regions: [],
  },
  {
    id: "middle-east",
    name: "중동",
    englishName: "Middle East",
    code: "ME",
    flag: "🇦🇪",
    regions: ["me-dubai", "me-jordan", "me-saudi", "me-oman", "me-tunisia"],
  },
  {
    id: "north-eu",
    name: "북유럽",
    englishName: "Northern Europe",
    code: "NEU",
    flag: "🇪🇺",
    regions: [
      "neu-norway",
      "neu-finland",
      "neu-denmark",
      "neu-sweden",
      "neu-lithuania",
      "neu-estonia",
      "neu-latvia",
      "neu-iceland",
    ],
  },
  {
    id: "africa",
    name: "아프리카",
    englishName: "Africa",
    code: "AF",
    flag: "🇦🇫",
    regions: ["af-kenya", "af-southafrica", "af-tanzania"],
  },
  {
    id: "mn",
    name: "몽골",
    englishName: "Mongolia",
    code: "MN",
    flag: "🇲🇳",
    regions: ["mn-ulaanbaatar"],
  },
  {
    id: "inner-mn",
    name: "내몽골",
    englishName: "Inner Mongolia",
    code: "IMN",
    flag: "🇨🇳",
    regions: ["imn-innermongolia"],
  },
  {
    id: "hk",
    name: "홍콩",
    englishName: "Hong Kong",
    code: "HK",
    flag: "🇭🇰",
    regions: [],
  },
  {
    id: "mo",
    name: "마카오",
    englishName: "Macau",
    code: "MO",
    flag: "🇲🇴",
    regions: [],
  },
  {
    id: "uz",
    name: "우즈베키스탄",
    englishName: "Uzbekistan",
    code: "UZ",
    flag: "🇺🇿",
    regions: ["uz-tashkent"],
  },
  {
    id: "kz",
    name: "카자흐스탄",
    englishName: "Kazakhstan",
    code: "KZ",
    flag: "🇰🇿",
    regions: ["kz-almaty"],
  },
  {
    id: "kg",
    name: "키르기스스탄",
    englishName: "Kyrgyzstan",
    code: "KG",
    flag: "🇰🇬",
    regions: ["kg-bishkek"],
  },
  {
    id: "au",
    name: "호주",
    englishName: "Australia",
    code: "AU",
    flag: "🇦🇺",
    regions: [
      "au-sydney",
      "au-melbourne",
      "au-brisbane",
      "au-perth",
      "au-adelaide",
      "au-canberra",
      "au-goldcoast",
      "au-cairns",
      "au-darwin",
      "au-hobart",
    ],
  },
  {
    id: "nz",
    name: "뉴질랜드",
    englishName: "New Zealand",
    code: "NZ",
    flag: "🇳🇿",
    regions: [
      "nz-auckland",
      "nz-queenstown",
      "nz-christchurch",
      "nz-rotorua",
      "nz-wellington",
      "nz-tekapo",
    ],
  },
  {
    id: "us",
    name: "미국",
    englishName: "United States",
    code: "US",
    flag: "🇺🇸",
    regions: [
      "us-newyork",
      "us-la",
      "us-sf",
      "us-lasvegas",
      "us-hawaii",
      "us-guam",
      "us-saipan",
      "us-seattle",
      "us-orlando",
      "us-chicago",
      "us-boston",
      "us-washington",
    ],
  },
  {
    id: "ca",
    name: "캐나다",
    englishName: "Canada",
    code: "CA",
    flag: "🇨🇦",
    regions: [
      "ca-vancouver",
      "ca-toronto",
      "ca-quebec",
      "ca-montreal",
      "ca-ottawa",
      "ca-banff",
      "ca-whistler",
      "ca-calgary",
      "ca-victoria",
      "ca-niagara",
    ],
  },
];

export const regionsList: Region[] = [
  // 대한민국
  {
    id: "kr-seoul",
    name: "서울",
    englishName: "Seoul",
    country: "대한민국",
    countryCode: "KR",
    flag: "🇰🇷",
    searchKeywords: ["서울", "seoul", "ㅅㅇ", "수도"],
  },
  {
    id: "kr-busan",
    name: "부산",
    englishName: "Busan",
    country: "대한민국",
    countryCode: "KR",
    flag: "🇰🇷",
    searchKeywords: ["부산", "busan", "ㅂㅅ", "해운대"],
  },
  {
    id: "kr-jeju",
    name: "제주도",
    englishName: "Jeju",
    country: "대한민국",
    countryCode: "KR",
    flag: "🇰🇷",
    searchKeywords: ["제주도", "jeju", "ㅈㅈ", "제주", "한라산"],
  },
  {
    id: "kr-gyeongju",
    name: "경주",
    englishName: "Gyeongju",
    country: "대한민국",
    countryCode: "KR",
    flag: "🇰🇷",
    searchKeywords: ["경주", "gyeongju", "ㄱㅈ", "불국사", "첨성대"],
  },
  {
    id: "kr-jeonju",
    name: "전주",
    englishName: "Jeonju",
    country: "대한민국",
    countryCode: "KR",
    flag: "🇰🇷",
    searchKeywords: ["전주", "jeonju", "ㅈㅈ", "한옥마을", "비빔밥"],
  },
  {
    id: "kr-gangneung",
    name: "강릉",
    englishName: "Gangneung",
    country: "대한민국",
    countryCode: "KR",
    flag: "🇰🇷",
    searchKeywords: ["강릉", "gangneung", "ㄱㄹ", "경포대", "커피"],
  },
  {
    id: "kr-yeosu",
    name: "여수",
    englishName: "Yeosu",
    country: "대한민국",
    countryCode: "KR",
    flag: "🇰🇷",
    searchKeywords: ["여수", "yeosu", "ㅇㅅ", "밤바다", "엑스포"],
  },
  {
    id: "kr-daegu",
    name: "대구",
    englishName: "Daegu",
    country: "대한민국",
    countryCode: "KR",
    flag: "🇰🇷",
    searchKeywords: ["대구", "daegu", "ㄷㄱ", "동성로"],
  },
  {
    id: "kr-incheon",
    name: "인천",
    englishName: "Incheon",
    country: "대한민국",
    countryCode: "KR",
    flag: "🇰🇷",
    searchKeywords: ["인천", "incheon", "ㅇㅊ", "공항", "차이나타운"],
  },
  {
    id: "kr-chuncheon",
    name: "춘천",
    englishName: "Chuncheon",
    country: "대한민국",
    countryCode: "KR",
    flag: "🇰🇷",
    searchKeywords: ["춘천", "chuncheon", "ㅊㅊ", "닭갈비", "남이섬"],
  },
  {
    id: "kr-andong",
    name: "안동",
    englishName: "Andong",
    country: "대한민국",
    countryCode: "KR",
    flag: "🇰🇷",
    searchKeywords: ["안동", "andong", "ㅇㄷ", "하회마을"],
  },
  {
    id: "kr-tongyeong",
    name: "통영",
    englishName: "Tongyeong",
    country: "대한민국",
    countryCode: "KR",
    flag: "🇰🇷",
    searchKeywords: ["통영", "tongyeong", "ㅌㅇ", "케이블카"],
  },
  {
    id: "kr-pohang",
    name: "포항",
    englishName: "Pohang",
    country: "대한민국",
    countryCode: "KR",
    flag: "🇰🇷",
    searchKeywords: ["포항", "pohang", "ㅍㅎ", "호미곶"],
  },
  {
    id: "kr-ulsan",
    name: "울산",
    englishName: "Ulsan",
    country: "대한민국",
    countryCode: "KR",
    flag: "🇰🇷",
    searchKeywords: ["울산", "ulsan", "ㅇㅅ", "태화강"],
  },
  {
    id: "kr-daejeon",
    name: "대전",
    englishName: "Daejeon",
    country: "대한민국",
    countryCode: "KR",
    flag: "🇰🇷",
    searchKeywords: ["대전", "daejeon", "ㄷㅈ", "과학도시"],
  },
  {
    id: "kr-gwangju",
    name: "광주",
    englishName: "Gwangju",
    country: "대한민국",
    countryCode: "KR",
    flag: "🇰🇷",
    searchKeywords: ["광주", "gwangju", "ㄱㅈ", "무등산"],
  },
  {
    id: "kr-sokcho",
    name: "속초",
    englishName: "Sokcho",
    country: "대한민국",
    countryCode: "KR",
    flag: "🇰🇷",
    searchKeywords: ["속초", "sokcho", "ㅅㅊ", "설악산", "아바이마을"],
  },
  {
    id: "kr-gapyeong",
    name: "가평",
    englishName: "Gapyeong",
    country: "대한민국",
    countryCode: "KR",
    flag: "🇰🇷",
    searchKeywords: ["가평", "gapyeong", "ㄱㅍ", "남이섬", "쁘띠프랑스"],
  },
  {
    id: "kr-namisum",
    name: "남이섬",
    englishName: "Nami Island",
    country: "대한민국",
    countryCode: "KR",
    flag: "🇰🇷",
    searchKeywords: ["남이섬", "nami", "ㄴㅇㅅ", "겨울연가"],
  },
  {
    id: "kr-seoraksan",
    name: "설악산",
    englishName: "Seoraksan",
    country: "대한민국",
    countryCode: "KR",
    flag: "🇰🇷",
    searchKeywords: ["설악산", "seoraksan", "ㅅㅇㅅ", "단풍", "케이블카"],
  },
  {
    id: "kr-jirisan",
    name: "지리산",
    englishName: "Jirisan",
    country: "대한민국",
    countryCode: "KR",
    flag: "🇰🇷",
    searchKeywords: ["지리산", "jirisan", "ㅈㄹㅅ", "천왕봉"],
  },
  {
    id: "kr-hallasan",
    name: "한라산",
    englishName: "Hallasan",
    country: "대한민국",
    countryCode: "KR",
    flag: "🇰🇷",
    searchKeywords: ["한라산", "hallasan", "ㅎㄹㅅ", "제주", "백록담"],
  },
  {
    id: "kr-taean",
    name: "태안",
    englishName: "Taean",
    country: "대한민국",
    countryCode: "KR",
    flag: "🇰🇷",
    searchKeywords: ["태안", "taean", "ㅌㅇ", "안면도"],
  },
  {
    id: "kr-boryeong",
    name: "보령",
    englishName: "Boryeong",
    country: "대한민국",
    countryCode: "KR",
    flag: "🇰🇷",
    searchKeywords: ["보령", "boryeong", "ㅂㄹ", "머드축제", "대천해수욕장"],
  },
  {
    id: "kr-danyang",
    name: "단양",
    englishName: "Danyang",
    country: "대한민국",
    countryCode: "KR",
    flag: "🇰🇷",
    searchKeywords: ["단양", "danyang", "ㄷㅇ", "도담삼봉"],
  },
  {
    id: "kr-yeongwol",
    name: "영월",
    englishName: "Yeongwol",
    country: "대한민국",
    countryCode: "KR",
    flag: "🇰🇷",
    searchKeywords: ["영월", "yeongwol", "ㅇㅇ", "별마로천문대"],
  },
  {
    id: "kr-jeongseon",
    name: "정선",
    englishName: "Jeongseon",
    country: "대한민국",
    countryCode: "KR",
    flag: "🇰🇷",
    searchKeywords: ["정선", "jeongseon", "ㅈㅅ", "아리랑"],
  },
  {
    id: "kr-pyeongchang",
    name: "평창",
    englishName: "Pyeongchang",
    country: "대한민국",
    countryCode: "KR",
    flag: "🇰🇷",
    searchKeywords: ["평창", "pyeongchang", "ㅍㅊ", "올림픽", "스키"],
  },
  {
    id: "kr-goseong",
    name: "고성",
    englishName: "Goseong",
    country: "대한민국",
    countryCode: "KR",
    flag: "🇰🇷",
    searchKeywords: ["고성", "goseong", "ㄱㅅ", "DMZ"],
  },
  {
    id: "kr-yangyang",
    name: "양양",
    englishName: "Yangyang",
    country: "대한민국",
    countryCode: "KR",
    flag: "🇰🇷",
    searchKeywords: ["양양", "yangyang", "ㅇㅇ", "낙산사", "서핑"],
  },
  {
    id: "kr-suncheon",
    name: "순천",
    englishName: "Suncheon",
    country: "대한민국",
    countryCode: "KR",
    flag: "🇰🇷",
    searchKeywords: ["순천", "suncheon", "ㅅㅊ", "습지", "정원"],
  },
  {
    id: "kr-damyang",
    name: "담양",
    englishName: "Damyang",
    country: "대한민국",
    countryCode: "KR",
    flag: "🇰🇷",
    searchKeywords: ["담양", "damyang", "ㄷㅇ", "죽녹원", "대나무"],
  },
  {
    id: "kr-hwasun",
    name: "화순",
    englishName: "Hwasun",
    country: "대한민국",
    countryCode: "KR",
    flag: "🇰🇷",
    searchKeywords: ["화순", "hwasun", "ㅎㅅ", "고인돌"],
  },
  {
    id: "kr-gokseong",
    name: "곡성",
    englishName: "Gokseong",
    country: "대한민국",
    countryCode: "KR",
    flag: "🇰🇷",
    searchKeywords: ["곡성", "gokseong", "ㄱㅅ", "기차마을"],
  },
  {
    id: "kr-gurye",
    name: "구례",
    englishName: "Gurye",
    country: "대한민국",
    countryCode: "KR",
    flag: "🇰🇷",
    searchKeywords: ["구례", "gurye", "ㄱㄹ", "산수유"],
  },

  // 태국
  {
    id: "th-bangkok",
    name: "방콕",
    englishName: "Bangkok",
    country: "태국",
    countryCode: "TH",
    flag: "🇹🇭",
    searchKeywords: ["방콕", "bangkok", "ㅂㅋ", "왓포", "카오산로드"],
  },
  {
    id: "th-pattaya",
    name: "파타야",
    englishName: "Pattaya",
    country: "태국",
    countryCode: "TH",
    flag: "🇹🇭",
    searchKeywords: ["파타야", "pattaya", "ㅍㅌㅇ", "워킹스트리트"],
  },
  {
    id: "th-phuket",
    name: "푸껫",
    englishName: "Phuket",
    country: "태국",
    countryCode: "TH",
    flag: "🇹🇭",
    searchKeywords: ["푸껫", "phuket", "ㅍㅋ", "파통비치"],
  },
  {
    id: "th-chiangmai",
    name: "치앙마이",
    englishName: "Chiang Mai",
    country: "태국",
    countryCode: "TH",
    flag: "🇹🇭",
    searchKeywords: ["치앙마이", "chiangmai", "ㅊㅇㅁㅇ", "도이수텝"],
  },
  {
    id: "th-ayutthaya",
    name: "아유타야",
    englishName: "Ayutthaya",
    country: "태국",
    countryCode: "TH",
    flag: "🇹🇭",
    searchKeywords: ["아유타야", "ayutthaya", "ㅇㅇㅌㅇ", "유적지"],
  },
  {
    id: "th-kanchanaburi",
    name: "칸차나부리",
    englishName: "Kanchanaburi",
    country: "태국",
    countryCode: "TH",
    flag: "🇹🇭",
    searchKeywords: ["칸차나부리", "kanchanaburi", "ㅋㅊㄴㅂㄹ", "콰이강다리"],
  },
  {
    id: "th-kohsamui",
    name: "코사무이",
    englishName: "Koh Samui",
    country: "태국",
    countryCode: "TH",
    flag: "🇹🇭",
    searchKeywords: ["코사무이", "kohsamui", "ㅋㅅㅁㅇ", "풀문파티"],
  },

  // 필리핀
  {
    id: "ph-boracay",
    name: "보라카이",
    englishName: "Boracay",
    country: "필리핀",
    countryCode: "PH",
    flag: "🇵🇭",
    searchKeywords: ["보라카이", "boracay", "ㅂㄹㅋㅇ", "화이트비치"],
  },
  {
    id: "ph-cebu",
    name: "세부",
    englishName: "Cebu",
    country: "필리핀",
    countryCode: "PH",
    flag: "🇵🇭",
    searchKeywords: ["세부", "cebu", "ㅅㅂ", "막탄섬"],
  },
  {
    id: "ph-bohol",
    name: "보홀",
    englishName: "Bohol",
    country: "필리핀",
    countryCode: "PH",
    flag: "🇵🇭",
    searchKeywords: ["보홀", "bohol", "ㅂㅎ", "초콜릿힐스"],
  },
  {
    id: "ph-manila",
    name: "마닐라",
    englishName: "Manila",
    country: "필리핀",
    countryCode: "PH",
    flag: "🇵🇭",
    searchKeywords: ["마닐라", "manila", "ㅁㄴㄹ", "인트라무로스"],
  },
  {
    id: "ph-clark",
    name: "클락",
    englishName: "Clark",
    country: "필리핀",
    countryCode: "PH",
    flag: "🇵🇭",
    searchKeywords: ["클락", "clark", "ㅋㄹ", "수빅베이"],
  },
  {
    id: "ph-palawan",
    name: "팔라완",
    englishName: "Palawan",
    country: "필리핀",
    countryCode: "PH",
    flag: "🇵🇭",
    searchKeywords: ["팔라완", "palawan", "ㅍㄹㅇ", "엘니도"],
  },

  // 베트남
  {
    id: "vn-danang",
    name: "다낭",
    englishName: "Da Nang",
    country: "베트남",
    countryCode: "VN",
    flag: "🇻🇳",
    searchKeywords: ["다낭", "danang", "ㄷㄴ", "바나힐", "용다리"],
  },
  {
    id: "vn-nhatrang",
    name: "나트랑",
    englishName: "Nha Trang",
    country: "베트남",
    countryCode: "VN",
    flag: "🇻🇳",
    searchKeywords: ["나트랑", "nhatrang", "ㄴㅌㄹ", "빈펄랜드"],
  },
  {
    id: "vn-dalat",
    name: "달랏",
    englishName: "Da Lat",
    country: "베트남",
    countryCode: "VN",
    flag: "🇻🇳",
    searchKeywords: ["달랏", "dalat", "ㄷㄹ", "꽃도시"],
  },
  {
    id: "vn-phuquoc",
    name: "푸꾸옥",
    englishName: "Phu Quoc",
    country: "베트남",
    countryCode: "VN",
    flag: "🇻🇳",
    searchKeywords: ["푸꾸옥", "phuquoc", "ㅍㄲㅇ", "케이블카"],
  },
  {
    id: "vn-hanoi",
    name: "하노이/하롱베이",
    englishName: "Hanoi/Ha Long Bay",
    country: "베트남",
    countryCode: "VN",
    flag: "🇻🇳",
    searchKeywords: ["하노이", "hanoi", "ㅎㄴㅇ", "하롱베이", "halongbay"],
  },
  {
    id: "vn-hochiminh",
    name: "호치민",
    englishName: "Ho Chi Minh",
    country: "베트남",
    countryCode: "VN",
    flag: "🇻🇳",
    searchKeywords: ["호치민", "hochiminh", "ㅎㅊㅁ", "사이공"],
  },
  {
    id: "vn-muine",
    name: "무이네",
    englishName: "Mui Ne",
    country: "베트남",
    countryCode: "VN",
    flag: "🇻🇳",
    searchKeywords: ["무이네", "muine", "ㅁㅇㄴ", "사막"],
  },
  {
    id: "vn-sapa",
    name: "사파",
    englishName: "Sapa",
    country: "베트남",
    countryCode: "VN",
    flag: "🇻🇳",
    searchKeywords: ["사파", "sapa", "ㅅㅍ", "계단식논"],
  },
  {
    id: "vn-donghoi",
    name: "동허이",
    englishName: "Dong Hoi",
    country: "베트남",
    countryCode: "VN",
    flag: "🇻🇳",
    searchKeywords: ["동허이", "donghoi", "ㄷㅎㅇ", "퐁냐케방"],
  },

  // 캄보디아
  {
    id: "kh-siemreap",
    name: "시엠립(앙코르와트)",
    englishName: "Siem Reap (Angkor Wat)",
    country: "캄보디아",
    countryCode: "KH",
    flag: "🇰🇭",
    searchKeywords: ["시엠립", "siemreap", "ㅅㅇㄹ", "앙코르와트", "angkorwat"],
  },
  {
    id: "kh-phnompenh",
    name: "프놈펜",
    englishName: "Phnom Penh",
    country: "캄보디아",
    countryCode: "KH",
    flag: "🇰🇭",
    searchKeywords: ["프놈펜", "phnompenh", "ㅍㄴㅍ", "왕궁"],
  },

  // 말레이시아
  {
    id: "my-kotakinabalu",
    name: "코타키나발루",
    englishName: "Kota Kinabalu",
    country: "말레이시아",
    countryCode: "MY",
    flag: "🇲🇾",
    searchKeywords: [
      "코타키나발루",
      "kotakinabalu",
      "ㅋㅌㅋㄴㅂㄹ",
      "키나발루산",
    ],
  },
  {
    id: "my-kualalumpur",
    name: "쿠알라룸푸르",
    englishName: "Kuala Lumpur",
    country: "말레이시아",
    countryCode: "MY",
    flag: "🇲🇾",
    searchKeywords: [
      "쿠알라룸푸르",
      "kualalumpur",
      "ㅋㅇㄹㄹㅍㄹ",
      "페트로나스타워",
    ],
  },

  // 인도네시아
  {
    id: "id-bali",
    name: "발리",
    englishName: "Bali",
    country: "인도네시아",
    countryCode: "ID",
    flag: "🇮🇩",
    searchKeywords: ["발리", "bali", "ㅂㄹ", "우붓", "꾸따"],
  },
  {
    id: "id-batam",
    name: "바탐",
    englishName: "Batam",
    country: "인도네시아",
    countryCode: "ID",
    flag: "🇮🇩",
    searchKeywords: ["바탐", "batam", "ㅂㅌ", "골프"],
  },
  {
    id: "id-manado",
    name: "마나도",
    englishName: "Manado",
    country: "인도네시아",
    countryCode: "ID",
    flag: "🇮🇩",
    searchKeywords: ["마나도", "manado", "ㅁㄴㄷ", "부나켄"],
  },

  // 라오스
  {
    id: "la-vangvieng",
    name: "방비엥",
    englishName: "Vang Vieng",
    country: "라오스",
    countryCode: "LA",
    flag: "🇱🇦",
    searchKeywords: ["방비엥", "vangvieng", "ㅂㅂㅇ", "카약"],
  },
  {
    id: "la-luangprabang",
    name: "루앙프라방",
    englishName: "Luang Prabang",
    country: "라오스",
    countryCode: "LA",
    flag: "🇱🇦",
    searchKeywords: ["루앙프라방", "luangprabang", "ㄹㅇㅍㄹㅂ", "왕궁박물관"],
  },
  {
    id: "la-vientiane",
    name: "비엔티안",
    englishName: "Vientiane",
    country: "라오스",
    countryCode: "LA",
    flag: "🇱🇦",
    searchKeywords: ["비엔티안", "vientiane", "ㅂㅇㅌㅇ", "왓시사켓"],
  },

  // 대만
  {
    id: "tw-taipei",
    name: "타이베이",
    englishName: "Taipei",
    country: "대만",
    countryCode: "TW",
    flag: "🇹🇼",
    searchKeywords: ["타이베이", "taipei", "ㅌㅇㅂㅇ", "101타워"],
  },
  {
    id: "tw-taichung",
    name: "타이중",
    englishName: "Taichung",
    country: "대만",
    countryCode: "TW",
    flag: "🇹🇼",
    searchKeywords: ["타이중", "taichung", "ㅌㅇㅈ", "일월담"],
  },
  {
    id: "tw-kaohsiung",
    name: "가오슝",
    englishName: "Kaohsiung",
    country: "대만",
    countryCode: "TW",
    flag: "🇹🇼",
    searchKeywords: ["가오슝", "kaohsiung", "ㄱㅇㅅ", "로투스연못"],
  },
  {
    id: "tw-tainan",
    name: "타이난",
    englishName: "Tainan",
    country: "대만",
    countryCode: "TW",
    flag: "🇹🇼",
    searchKeywords: ["타이난", "tainan", "ㅌㅇㄴ", "적간루"],
  },

  // 중국
  {
    id: "cn-zhangjiajie",
    name: "장가계",
    englishName: "Zhangjiajie",
    country: "중국",
    countryCode: "CN",
    flag: "🇨🇳",
    searchKeywords: ["장가계", "zhangjiajie", "ㅈㄱㄱ", "아바타산"],
  },
  {
    id: "cn-changbaishan",
    name: "백두산",
    englishName: "Changbai Mountain",
    country: "중국",
    countryCode: "CN",
    flag: "🇨🇳",
    searchKeywords: ["백두산", "changbaishan", "ㅂㄷㅅ", "천지"],
  },
  {
    id: "cn-shanghai",
    name: "상해",
    englishName: "Shanghai",
    country: "중국",
    countryCode: "CN",
    flag: "🇨🇳",
    searchKeywords: ["상해", "shanghai", "ㅅㅎ", "와이탄"],
  },
  {
    id: "cn-beijing",
    name: "북경",
    englishName: "Beijing",
    country: "중국",
    countryCode: "CN",
    flag: "🇨🇳",
    searchKeywords: ["북경", "beijing", "ㅂㄱ", "만리장성", "자금성"],
  },
  {
    id: "cn-chengdu",
    name: "성도",
    englishName: "Chengdu",
    country: "중국",
    countryCode: "CN",
    flag: "🇨🇳",
    searchKeywords: ["성도", "chengdu", "ㅅㄷ", "판다"],
  },
  {
    id: "cn-jiuzhaigou",
    name: "구채구",
    englishName: "Jiuzhaigou",
    country: "중국",
    countryCode: "CN",
    flag: "🇨🇳",
    searchKeywords: ["구채구", "jiuzhaigou", "ㄱㅊㄱ", "칠채못"],
  },
  {
    id: "cn-guiyang",
    name: "귀양",
    englishName: "Guiyang",
    country: "중국",
    countryCode: "CN",
    flag: "🇨🇳",
    searchKeywords: ["귀양", "guiyang", "ㄱㅇ", "황과수폭포"],
  },
  {
    id: "cn-anshun",
    name: "안순",
    englishName: "Anshun",
    country: "중국",
    countryCode: "CN",
    flag: "🇨🇳",
    searchKeywords: ["안순", "anshun", "ㅇㅅ", "황과수"],
  },
  {
    id: "cn-tibet",
    name: "티벳",
    englishName: "Tibet",
    country: "중국",
    countryCode: "CN",
    flag: "🇨🇳",
    searchKeywords: ["티벳", "tibet", "ㅌㅂ", "라싸", "포탈라궁"],
  },
  {
    id: "cn-chongqing",
    name: "중경",
    englishName: "Chongqing",
    country: "중국",
    countryCode: "CN",
    flag: "🇨🇳",
    searchKeywords: ["중경", "chongqing", "ㅈㄱ", "홍애동"],
  },
  {
    id: "cn-harbin",
    name: "하얼빈",
    englishName: "Harbin",
    country: "중국",
    countryCode: "CN",
    flag: "🇨🇳",
    searchKeywords: ["하얼빈", "harbin", "ㅎㅇㅂ", "빙등축제"],
  },
  {
    id: "cn-huangshan",
    name: "황산",
    englishName: "Huangshan",
    country: "중국",
    countryCode: "CN",
    flag: "🇨🇳",
    searchKeywords: ["황산", "huangshan", "ㅎㅅ", "영주"],
  },
  {
    id: "cn-xian",
    name: "서안",
    englishName: "Xian",
    country: "중국",
    countryCode: "CN",
    flag: "🇨🇳",
    searchKeywords: ["서안", "xian", "ㅅㅇ", "병마용"],
  },
  {
    id: "cn-dalian",
    name: "대련",
    englishName: "Dalian",
    country: "중국",
    countryCode: "CN",
    flag: "🇨🇳",
    searchKeywords: ["대련", "dalian", "ㄷㄹ", "성해광장"],
  },
  {
    id: "cn-qingdao",
    name: "신선거",
    englishName: "Qingdao",
    country: "중국",
    countryCode: "CN",
    flag: "🇨🇳",
    searchKeywords: ["신선거", "qingdao", "ㅅㅅㄱ", "청도맥주"],
  },
  {
    id: "cn-yantai",
    name: "연태",
    englishName: "Yantai",
    country: "중국",
    countryCode: "CN",
    flag: "🇨🇳",
    searchKeywords: ["연태", "yantai", "ㅇㅌ", "장도"],
  },
  {
    id: "cn-jinan",
    name: "제남",
    englishName: "Jinan",
    country: "중국",
    countryCode: "CN",
    flag: "🇨🇳",
    searchKeywords: ["제남", "jinan", "ㅈㄴ", "천불산"],
  },
  {
    id: "cn-weihai",
    name: "위해",
    englishName: "Weihai",
    country: "중국",
    countryCode: "CN",
    flag: "🇨🇳",
    searchKeywords: ["위해", "weihai", "ㅇㅎ", "유공도"],
  },
  {
    id: "cn-hainan",
    name: "하이난",
    englishName: "Hainan",
    country: "중국",
    countryCode: "CN",
    flag: "🇨🇳",
    searchKeywords: ["하이난", "hainan", "ㅎㅇㄴ", "유공도"],
  },

  // 일본 (전체 지역)
  {
    id: "jp-tokyo",
    name: "도쿄",
    englishName: "Tokyo",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["도쿄", "tokyo", "ㄷㅋ", "시부야", "아사쿠사"],
  },
  {
    id: "jp-shizuoka",
    name: "시즈오카",
    englishName: "Shizuoka",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["시즈오카", "shizuoka", "ㅅㅈㅇㅋ", "후지산"],
  },
  {
    id: "jp-izu",
    name: "이즈",
    englishName: "Izu",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["이즈", "izu", "ㅇㅈ", "온천"],
  },
  {
    id: "jp-hakone",
    name: "하코네",
    englishName: "Hakone",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["하코네", "hakone", "ㅎㅋㄴ", "온천", "아시노코"],
  },
  {
    id: "jp-fuji",
    name: "후지산",
    englishName: "Fuji",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["후지산", "fuji", "ㅎㅈㅅ", "후지", "등반"],
  },
  {
    id: "jp-yokohama",
    name: "요코하마",
    englishName: "Yokohama",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["요코하마", "yokohama", "ㅇㅋㅎㅁ", "미나토미라이"],
  },
  {
    id: "jp-kamakura",
    name: "가마쿠라",
    englishName: "Kamakura",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["가마쿠라", "kamakura", "ㄱㅁㅋㄹ", "대불"],
  },
  {
    id: "jp-yamanashi",
    name: "야마나시",
    englishName: "Yamanashi",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["야마나시", "yamanashi", "ㅇㅁㄴㅅ", "포도"],
  },
  {
    id: "jp-osaka",
    name: "오사카",
    englishName: "Osaka",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["오사카", "osaka", "ㅇㅅㅋ", "도톤보리", "유니버셜"],
  },
  {
    id: "jp-kyoto",
    name: "교토",
    englishName: "Kyoto",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["교토", "kyoto", "ㄱㅌ", "기요미즈데라", "기온"],
  },
  {
    id: "jp-kobe",
    name: "고베",
    englishName: "Kobe",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["고베", "kobe", "ㄱㅂ", "고베규", "항구"],
  },
  {
    id: "jp-wakayama",
    name: "와카야마",
    englishName: "Wakayama",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["와카야마", "wakayama", "ㅇㅋㅇㅁ", "구마노"],
  },
  {
    id: "jp-sarayama",
    name: "사라야마",
    englishName: "Sarayama",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["사라야마", "sarayama", "ㅅㄹㅇㅁ"],
  },
  {
    id: "jp-fukuoka",
    name: "후쿠오카",
    englishName: "Fukuoka",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["후쿠오카", "fukuoka", "ㅎㅋㅇㅋ", "하카타", "라멘"],
  },
  {
    id: "jp-beppu",
    name: "벳푸",
    englishName: "Beppu",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["벳푸", "beppu", "ㅂㅍ", "온천", "지옥순회"],
  },
  {
    id: "jp-yufuin",
    name: "유후인",
    englishName: "Yufuin",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["유후인", "yufuin", "ㅇㅎㅇ", "온천"],
  },
  {
    id: "jp-saga",
    name: "사가",
    englishName: "Saga",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["사가", "saga", "ㅅㄱ", "아리타"],
  },
  {
    id: "jp-oita",
    name: "오이타",
    englishName: "Oita",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["오이타", "oita", "ㅇㅇㅌ", "온천"],
  },
  {
    id: "jp-kumamoto",
    name: "구마모토",
    englishName: "Kumamoto",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["구마모토", "kumamoto", "ㄱㅁㅁㅌ", "구마몬", "성"],
  },
  {
    id: "jp-nagasaki",
    name: "나가사키",
    englishName: "Nagasaki",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["나가사키", "nagasaki", "ㄴㄱㅅㅋ", "글로버가든"],
  },
  {
    id: "jp-kagoshima",
    name: "가고시마",
    englishName: "Kagoshima",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["가고시마", "kagoshima", "ㄱㄱㅅㅁ", "사쿠라지마"],
  },
  {
    id: "jp-miyazaki",
    name: "미야자키",
    englishName: "Miyazaki",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["미야자키", "miyazaki", "ㅁㅇㅈㅋ", "망고"],
  },
  {
    id: "jp-kitakyushu",
    name: "기타규슈",
    englishName: "Kitakyushu",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["기타규슈", "kitakyushu", "ㄱㅌㄱㅅ"],
  },
  {
    id: "jp-yamaguchi",
    name: "야마구치",
    englishName: "Yamaguchi",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["야마구치", "yamaguchi", "ㅇㅁㄱㅊ", "시모노세키"],
  },
  {
    id: "jp-sapporo",
    name: "삿포로",
    englishName: "Sapporo",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["삿포로", "sapporo", "ㅅㅍㄹ", "맥주", "눈축제"],
  },
  {
    id: "jp-furano",
    name: "후라노",
    englishName: "Furano",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["후라노", "furano", "ㅎㄹㄴ", "라벤더", "스키"],
  },
  {
    id: "jp-biei",
    name: "비에이",
    englishName: "Biei",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["비에이", "biei", "ㅂㅇㅇ", "꽃밭"],
  },
  {
    id: "jp-asahikawa",
    name: "아사히카와",
    englishName: "Asahikawa",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["아사히카와", "asahikawa", "ㅇㅅㅎㅋㅇ", "동물원"],
  },
  {
    id: "jp-noboribetsu",
    name: "노보리베츠",
    englishName: "Noboribetsu",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["노보리베츠", "noboribetsu", "ㄴㅂㄹㅂㅊ", "온천"],
  },
  {
    id: "jp-hakodate",
    name: "하코다테",
    englishName: "Hakodate",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["하코다테", "hakodate", "ㅎㅋㄷㅌ", "야경", "아침시장"],
  },
  {
    id: "jp-takamatsu",
    name: "다카마츠",
    englishName: "Takamatsu",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["다카마츠", "takamatsu", "ㄷㅋㅁㅊ", "우동"],
  },
  {
    id: "jp-naoshima",
    name: "나오시마",
    englishName: "Naoshima",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["나오시마", "naoshima", "ㄴㅇㅅㅁ", "예술섬"],
  },
  {
    id: "jp-matsuyama",
    name: "마츠야마",
    englishName: "Matsuyama",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["마츠야마", "matsuyama", "ㅁㅊㅇㅁ", "도고온천"],
  },
  {
    id: "jp-tokushima",
    name: "도쿠시마",
    englishName: "Tokushima",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["도쿠시마", "tokushima", "ㄷㅋㅅㅁ", "아와오도리"],
  },
  {
    id: "jp-tottori",
    name: "돗토리",
    englishName: "Tottori",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["돗토리", "tottori", "ㄷㅌㄹ", "사구"],
  },
  {
    id: "jp-yonago",
    name: "요나고",
    englishName: "Yonago",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["요나고", "yonago", "ㅇㄴㄱ", "다이센"],
  },
  {
    id: "jp-hiroshima",
    name: "히로시마",
    englishName: "Hiroshima",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: [
      "히로시마",
      "hiroshima",
      "ㅎㄹㅅㅁ",
      "평화공원",
      "미야지마",
    ],
  },
  {
    id: "jp-okayama",
    name: "오카야마",
    englishName: "Okayama",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["오카야마", "okayama", "ㅇㅋㅇㅁ", "고라쿠엔"],
  },
  {
    id: "jp-okinawa",
    name: "오키나와",
    englishName: "Okinawa",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["오키나와", "okinawa", "ㅇㅋㄴㅇ", "나하", "추라우미"],
  },
  {
    id: "jp-ishigaki",
    name: "이시가키",
    englishName: "Ishigaki",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["이시가키", "ishigaki", "ㅇㅅㄱㅋ", "다이빙"],
  },
  {
    id: "jp-toyama",
    name: "도야마",
    englishName: "Toyama",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["도야마", "toyama", "ㄷㅇㅁ", "다테야마"],
  },
  {
    id: "jp-alpenroute",
    name: "알펜루트",
    englishName: "Alpen Route",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["알펜루트", "alpenroute", "ㅇㅍㄹㅌ", "다테야마쿠로베"],
  },
  {
    id: "jp-nagoya",
    name: "나고야",
    englishName: "Nagoya",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["나고야", "nagoya", "ㄴㄱㅇ", "성"],
  },
  {
    id: "jp-takayama",
    name: "다카야마",
    englishName: "Takayama",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["다카야마", "takayama", "ㄷㅋㅇㅁ", "히다규"],
  },
  {
    id: "jp-komatsu",
    name: "고마츠",
    englishName: "Komatsu",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["고마츠", "komatsu", "ㄱㅁㅊ", "가나자와"],
  },
  {
    id: "jp-niigata",
    name: "니가타",
    englishName: "Niigata",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["니가타", "niigata", "ㄴㄱㅌ", "사케", "쌀"],
  },
  {
    id: "jp-aomori",
    name: "아오모리",
    englishName: "Aomori",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["아오모리", "aomori", "ㅇㅇㅁㄹ", "사과"],
  },
  {
    id: "jp-akita",
    name: "아키타",
    englishName: "Akita",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["아키타", "akita", "ㅇㅋㅌ", "나마하게"],
  },
  {
    id: "jp-sendai",
    name: "센다이",
    englishName: "Sendai",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["센다이", "sendai", "ㅅㄷㅇ", "규탄"],
  },
  {
    id: "jp-yamagata",
    name: "야마가타",
    englishName: "Yamagata",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["야마가타", "yamagata", "ㅇㅁㄱㅌ", "체리"],
  },

  // 서유럽 (전체 지역)
  {
    id: "weu-uk",
    name: "영국",
    englishName: "United Kingdom",
    country: "서유럽",
    countryCode: "WEU",
    flag: "🇬🇧",
    searchKeywords: ["영국", "uk", "ㅇㄱ", "런던", "빅벤"],
  },
  {
    id: "weu-switzerland",
    name: "스위스",
    englishName: "Switzerland",
    country: "서유럽",
    countryCode: "WEU",
    flag: "🇨🇭",
    searchKeywords: ["스위스", "switzerland", "ㅅㅇㅅ", "융프라우", "마터호른"],
  },
  {
    id: "weu-italy",
    name: "이탈리아",
    englishName: "Italy",
    country: "서유럽",
    countryCode: "WEU",
    flag: "🇮🇹",
    searchKeywords: ["이탈리아", "italy", "ㅇㅌㄹㅇ", "로마", "베니스"],
  },
  {
    id: "weu-france",
    name: "프랑스",
    englishName: "France",
    country: "서유럽",
    countryCode: "WEU",
    flag: "🇫🇷",
    searchKeywords: ["프랑스", "france", "ㅍㄹㅅ", "파리", "에펠탑"],
  },
  {
    id: "weu-netherlands",
    name: "네덜란드",
    englishName: "Netherlands",
    country: "서유럽",
    countryCode: "WEU",
    flag: "🇳🇱",
    searchKeywords: [
      "네덜란드",
      "netherlands",
      "ㄴㄷㄹㄷ",
      "암스테르담",
      "튤립",
    ],
  },
  {
    id: "weu-belgium",
    name: "벨기에",
    englishName: "Belgium",
    country: "서유럽",
    countryCode: "WEU",
    flag: "🇧🇪",
    searchKeywords: ["벨기에", "belgium", "ㅂㄱㅇ", "브뤼셀", "와플"],
  },
  {
    id: "weu-romania",
    name: "루마니아",
    englishName: "Romania",
    country: "서유럽",
    countryCode: "WEU",
    flag: "🇷🇴",
    searchKeywords: [
      "루마니아",
      "romania",
      "ㄹㅁㄴㅇ",
      "부카레스트",
      "드라큘라",
    ],
  },
  {
    id: "weu-germany",
    name: "독일",
    englishName: "Germany",
    country: "서유럽",
    countryCode: "WEU",
    flag: "🇩🇪",
    searchKeywords: ["독일", "germany", "ㄷㅇ", "베를린", "맥주"],
  },
  {
    id: "weu-ireland",
    name: "아일랜드",
    englishName: "Ireland",
    country: "서유럽",
    countryCode: "WEU",
    flag: "🇮🇪",
    searchKeywords: ["아일랜드", "ireland", "ㅇㅇㄹㄷ", "더블린", "클리프"],
  },

  // 동유럽 (전체 지역)
  {
    id: "eeu-austria",
    name: "오스트리아",
    englishName: "Austria",
    country: "동유럽",
    countryCode: "EEU",
    flag: "🇦🇹",
    searchKeywords: ["오스트리아", "austria", "ㅇㅅㅌㄹㅇ", "빈", "잘츠부르크"],
  },
  {
    id: "eeu-czech",
    name: "체코",
    englishName: "Czech Republic",
    country: "동유럽",
    countryCode: "EEU",
    flag: "🇨🇿",
    searchKeywords: ["체코", "czech", "ㅊㅋ", "프라하", "카를교"],
  },
  {
    id: "eeu-hungary",
    name: "헝가리",
    englishName: "Hungary",
    country: "동유럽",
    countryCode: "EEU",
    flag: "🇭🇺",
    searchKeywords: ["헝가리", "hungary", "ㅎㄱㄹ", "부다페스트", "온천"],
  },

  // 중동 (전체 지역)
  {
    id: "me-dubai",
    name: "두바이",
    englishName: "Dubai",
    country: "중동",
    countryCode: "ME",
    flag: "🇦🇪",
    searchKeywords: ["두바이", "dubai", "ㄷㅂㅇ", "부르즈칼리파"],
  },
  {
    id: "me-jordan",
    name: "요르단",
    englishName: "Jordan",
    country: "중동",
    countryCode: "ME",
    flag: "🇯🇴",
    searchKeywords: ["요르단", "jordan", "ㅇㄹㄷ", "페트라"],
  },
  {
    id: "me-saudi",
    name: "사우디아라비아",
    englishName: "Saudi Arabia",
    country: "중동",
    countryCode: "ME",
    flag: "🇸🇦",
    searchKeywords: [
      "사우디아라비아",
      "saudi",
      "ㅅㅇㄷㅇㄹㅂㅇ",
      "리야드",
      "메카",
    ],
  },
  {
    id: "me-oman",
    name: "오만",
    englishName: "Oman",
    country: "중동",
    countryCode: "ME",
    flag: "🇴🇲",
    searchKeywords: ["오만", "oman", "ㅇㅁ", "무스카트", "사막"],
  },
  {
    id: "me-tunisia",
    name: "튀니지",
    englishName: "Tunisia",
    country: "중동",
    countryCode: "ME",
    flag: "🇹🇳",
    searchKeywords: ["튀니지", "tunisia", "ㅌㄴㅈ", "튀니스", "사하라"],
  },

  // 북유럽 (전체 지역)
  {
    id: "neu-norway",
    name: "노르웨이",
    englishName: "Norway",
    country: "북유럽",
    countryCode: "NEU",
    flag: "🇳🇴",
    searchKeywords: ["노르웨이", "norway", "ㄴㄹㅇㅇ", "오로라", "피오르드"],
  },
  {
    id: "neu-finland",
    name: "핀란드",
    englishName: "Finland",
    country: "북유럽",
    countryCode: "NEU",
    flag: "🇫🇮",
    searchKeywords: ["핀란드", "finland", "ㅍㄹㄷ", "헬싱키", "사우나"],
  },
  {
    id: "neu-denmark",
    name: "덴마크",
    englishName: "Denmark",
    country: "북유럽",
    countryCode: "NEU",
    flag: "🇩🇰",
    searchKeywords: ["덴마크", "denmark", "ㄷㅁㅋ", "코펜하겐", "레고"],
  },
  {
    id: "neu-sweden",
    name: "스웨덴",
    englishName: "Sweden",
    country: "북유럽",
    countryCode: "NEU",
    flag: "🇸🇪",
    searchKeywords: ["스웨덴", "sweden", "ㅅㅇㄷ", "스톡홀름", "이케아"],
  },
  {
    id: "neu-lithuania",
    name: "리투아니아",
    englishName: "Lithuania",
    country: "북유럽",
    countryCode: "NEU",
    flag: "🇱🇹",
    searchKeywords: ["리투아니아", "lithuania", "ㄹㅌㅇㄴㅇ", "빌뉴스"],
  },
  {
    id: "neu-estonia",
    name: "에스토니아",
    englishName: "Estonia",
    country: "북유럽",
    countryCode: "NEU",
    flag: "🇪🇪",
    searchKeywords: ["에스토니아", "estonia", "ㅇㅅㅌㄴㅇ", "탈린"],
  },
  {
    id: "neu-latvia",
    name: "라트비아",
    englishName: "Latvia",
    country: "북유럽",
    countryCode: "NEU",
    flag: "🇱🇻",
    searchKeywords: ["라트비아", "latvia", "ㄹㅌㅂㅇ", "리가"],
  },
  {
    id: "neu-iceland",
    name: "아이슬란드",
    englishName: "Iceland",
    country: "북유럽",
    countryCode: "NEU",
    flag: "🇮🇸",
    searchKeywords: ["아이슬란드", "iceland", "ㅇㅇㅅㄹㄷ", "블루라군"],
  },

  // 아프리카
  {
    id: "af-kenya",
    name: "케냐",
    englishName: "Kenya",
    country: "아프리카",
    countryCode: "AF",
    flag: "🇰🇪",
    searchKeywords: ["케냐", "kenya", "ㅋㄴ", "사파리"],
  },
  {
    id: "af-southafrica",
    name: "남아공",
    englishName: "South Africa",
    country: "아프리카",
    countryCode: "AF",
    flag: "🇿🇦",
    searchKeywords: ["남아공", "southafrica", "ㄴㅇㄱ", "케이프타운"],
  },
  {
    id: "af-tanzania",
    name: "탄자니아",
    englishName: "Tanzania",
    country: "아프리카",
    countryCode: "AF",
    flag: "🇹🇿",
    searchKeywords: [
      "탄자니아",
      "tanzania",
      "ㅌㅈㄴㅇ",
      "킬리만자로",
      "세렝게티",
    ],
  },

  // 몽골
  {
    id: "mn-ulaanbaatar",
    name: "울란바토르",
    englishName: "Ulaanbaatar",
    country: "몽골",
    countryCode: "MN",
    flag: "🇲🇳",
    searchKeywords: ["울란바토르", "ulaanbaatar", "ㅇㄹㅂㅌㄹ", "게르"],
  },
  {
    id: "imn-innermongolia",
    name: "내몽고",
    englishName: "Inner Mongolia",
    country: "내몽골",
    countryCode: "IMN",
    flag: "🇨🇳",
    searchKeywords: ["내몽고", "innermongolia", "ㄴㅁㄱ", "초원"],
  },

  // 기타 중앙아시아
  {
    id: "uz-tashkent",
    name: "타슈켄트",
    englishName: "Tashkent",
    country: "우즈베키스탄",
    countryCode: "UZ",
    flag: "🇺🇿",
    searchKeywords: ["타슈켄트", "tashkent", "ㅌㅅㅋㅌ", "실크로드"],
  },
  {
    id: "kz-almaty",
    name: "알마티",
    englishName: "Almaty",
    country: "카자흐스탄",
    countryCode: "KZ",
    flag: "🇰🇿",
    searchKeywords: ["알마티", "almaty", "ㅇㅁㅌ", "메데우"],
  },
  {
    id: "kg-bishkek",
    name: "비슈케크",
    englishName: "Bishkek",
    country: "키르기스스탄",
    countryCode: "KG",
    flag: "🇰🇬",
    searchKeywords: ["비슈케크", "bishkek", "ㅂㅅㅋㅋ", "알라투"],
  },

  // 호주
  {
    id: "au-sydney",
    name: "시드니",
    englishName: "Sydney",
    country: "호주",
    countryCode: "AU",
    flag: "🇦🇺",
    searchKeywords: ["시드니", "sydney", "ㅅㄷㄴ", "오페라하우스"],
  },
  {
    id: "au-melbourne",
    name: "멜버른",
    englishName: "Melbourne",
    country: "호주",
    countryCode: "AU",
    flag: "🇦🇺",
    searchKeywords: ["멜버른", "melbourne", "ㅁㅂㄹ"],
  },
  {
    id: "au-brisbane",
    name: "브리즈번",
    englishName: "Brisbane",
    country: "호주",
    countryCode: "AU",
    flag: "🇦🇺",
    searchKeywords: ["브리즈번", "brisbane", "ㅂㄹㅈㅂ", "골드코스트"],
  },
  {
    id: "au-canberra",
    name: "캔버라",
    englishName: "Canberra",
    country: "호주",
    countryCode: "AU",
    flag: "🇦🇺",
    searchKeywords: ["캔버라", "canberra", "ㅋㅂㄹ"],
  },
  {
    id: "au-goldcoast",
    name: "골드코스트",
    englishName: "Gold Coast",
    country: "호주",
    countryCode: "AU",
    flag: "🇦🇺",
    searchKeywords: ["골드코스트", "goldcoast", "ㄱㄷㅋㅅㅌ"],
  },

  // 뉴질랜드
  {
    id: "nz-auckland",
    name: "오클랜드",
    englishName: "Auckland",
    country: "뉴질랜드",
    countryCode: "NZ",
    flag: "🇳🇿",
    searchKeywords: ["오클랜드", "auckland", "ㅇㅋㄹㄷ", "스카이타워"],
  },
  {
    id: "nz-queenstown",
    name: "퀸스타운",
    englishName: "Queenstown",
    country: "뉴질랜드",
    countryCode: "NZ",
    flag: "🇳🇿",
    searchKeywords: [
      "퀸스타운",
      "queenstown",
      "ㅋㅅㅌㅇ",
      "액티비티",
      "반지의제왕",
    ],
  },
  {
    id: "nz-christchurch",
    name: "크라이스트처치",
    englishName: "Christchurch",
    country: "뉴질랜드",
    countryCode: "NZ",
    flag: "🇳🇿",
    searchKeywords: [
      "크라이스트처치",
      "christchurch",
      "ㅋㄹㅇㅅㅌㅊㅊ",
      "가든시티",
    ],
  },
  {
    id: "nz-rotorua",
    name: "로토루아",
    englishName: "Rotorua",
    country: "뉴질랜드",
    countryCode: "NZ",
    flag: "🇳🇿",
    searchKeywords: ["로토루아", "rotorua", "ㄹㅌㄹㅇ", "온천", "마오리"],
  },
  {
    id: "nz-wellington",
    name: "웰링턴",
    englishName: "Wellington",
    country: "뉴질랜드",
    countryCode: "NZ",
    flag: "🇳🇿",
    searchKeywords: ["웰링턴", "wellington", "ㅇㄹㅌ", "수도", "바람의도시"],
  },
  {
    id: "nz-tekapo",
    name: "테카포",
    englishName: "Tekapo",
    country: "뉴질랜드",
    countryCode: "NZ",
    flag: "🇳🇿",
    searchKeywords: ["테카포", "tekapo", "ㅌㅋㅍ", "별관측", "호수"],
  },

  // 스페인
  {
    id: "es-barcelona",
    name: "바르셀로나",
    englishName: "Barcelona",
    country: "스페인",
    countryCode: "ES",
    flag: "🇪🇸",
    searchKeywords: [
      "바르셀로나",
      "barcelona",
      "ㅂㄹㅅㄹㄴ",
      "가우디",
      "사그라다파밀리아",
    ],
  },
  {
    id: "es-madrid",
    name: "마드리드",
    englishName: "Madrid",
    country: "스페인",
    countryCode: "ES",
    flag: "🇪🇸",
    searchKeywords: ["마드리드", "madrid", "ㅁㄷㄹㄷ", "왕궁", "프라도"],
  },
  {
    id: "es-sevilla",
    name: "세비야",
    englishName: "Sevilla",
    country: "스페인",
    countryCode: "ES",
    flag: "🇪🇸",
    searchKeywords: ["세비야", "sevilla", "ㅅㅂㅇ", "플라멩코", "스페인광장"],
  },
  {
    id: "es-granada",
    name: "그라나다",
    englishName: "Granada",
    country: "스페인",
    countryCode: "ES",
    flag: "🇪🇸",
    searchKeywords: ["그라나다", "granada", "ㄱㄹㄴㄷ", "알함브라", "궁전"],
  },
  {
    id: "es-valencia",
    name: "발렌시아",
    englishName: "Valencia",
    country: "스페인",
    countryCode: "ES",
    flag: "🇪🇸",
    searchKeywords: [
      "발렌시아",
      "valencia",
      "ㅂㄹㅅㅇ",
      "빠에야",
      "예술과학도시",
    ],
  },
  {
    id: "es-mallorca",
    name: "마요르카",
    englishName: "Mallorca",
    country: "스페인",
    countryCode: "ES",
    flag: "🇪🇸",
    searchKeywords: ["마요르카", "mallorca", "ㅁㅇㄹㅋ", "휴양지", "해변"],
  },

  // 포르투갈
  {
    id: "pt-lisbon",
    name: "리스본",
    englishName: "Lisbon",
    country: "포르투갈",
    countryCode: "PT",
    flag: "🇵🇹",
    searchKeywords: ["리스본", "lisbon", "ㄹㅅㅂ", "트램", "에그타르트"],
  },
  {
    id: "pt-porto",
    name: "포르투",
    englishName: "Porto",
    country: "포르투갈",
    countryCode: "PT",
    flag: "🇵🇹",
    searchKeywords: ["포르투", "porto", "ㅍㄹㅌ", "와인", "동루이스다리"],
  },
  {
    id: "pt-faro",
    name: "파로",
    englishName: "Faro",
    country: "포르투갈",
    countryCode: "PT",
    flag: "🇵🇹",
    searchKeywords: ["파로", "faro", "ㅍㄹ", "알가르베", "해변"],
  },

  // 미국
  {
    id: "us-newyork",
    name: "뉴욕",
    englishName: "New York",
    country: "미국",
    countryCode: "US",
    flag: "🇺🇸",
    searchKeywords: [
      "뉴욕",
      "newyork",
      "nyc",
      "ㄴㅇ",
      "타임스퀘어",
      "자유의여신상",
    ],
  },
  {
    id: "us-la",
    name: "로스앤젤레스",
    englishName: "Los Angeles",
    country: "미국",
    countryCode: "US",
    flag: "🇺🇸",
    searchKeywords: [
      "로스앤젤레스",
      "losangeles",
      "la",
      "ㄹㅇ",
      "할리우드",
      "디즈니랜드",
    ],
  },
  {
    id: "us-sf",
    name: "샌프란시스코",
    englishName: "San Francisco",
    country: "미국",
    countryCode: "US",
    flag: "🇺🇸",
    searchKeywords: [
      "샌프란시스코",
      "sanfrancisco",
      "sf",
      "ㅅㅍㄹ",
      "금문교",
      "실리콘밸리",
    ],
  },
  {
    id: "us-lasvegas",
    name: "라스베이거스",
    englishName: "Las Vegas",
    country: "미국",
    countryCode: "US",
    flag: "🇺🇸",
    searchKeywords: [
      "라스베이거스",
      "lasvegas",
      "ㄹㅅㅂ",
      "그랜드캐년",
      "카지노",
    ],
  },
  {
    id: "us-hawaii",
    name: "하와이",
    englishName: "Hawaii",
    country: "미국",
    countryCode: "US",
    flag: "🇺🇸",
    searchKeywords: [
      "하와이",
      "hawaii",
      "ㅎㅇㅇ",
      "와이키키",
      "호놀룰루",
      "신혼여행",
    ],
  },
  {
    id: "us-guam",
    name: "괌",
    englishName: "Guam",
    country: "미국",
    countryCode: "US",
    flag: "🇺🇸",
    searchKeywords: ["괌", "guam", "ㄱ", "투몬", "휴양지"],
  },
  {
    id: "us-saipan",
    name: "사이판",
    englishName: "Saipan",
    country: "미국",
    countryCode: "US",
    flag: "🇺🇸",
    searchKeywords: ["사이판", "saipan", "ㅅㅇㅍ", "마나가하", "그로토"],
  },
  {
    id: "us-seattle",
    name: "시애틀",
    englishName: "Seattle",
    country: "미국",
    countryCode: "US",
    flag: "🇺🇸",
    searchKeywords: ["시애틀", "seattle", "ㅅㅇㅌ", "스타벅스", "스페이스니들"],
  },
  {
    id: "us-orlando",
    name: "올랜도",
    englishName: "Orlando",
    country: "미국",
    countryCode: "US",
    flag: "🇺🇸",
    searchKeywords: ["올랜도", "orlando", "ㅇㄹㄷ", "디즈니월드", "유니버셜"],
  },
  {
    id: "us-chicago",
    name: "시카고",
    englishName: "Chicago",
    country: "미국",
    countryCode: "US",
    flag: "🇺🇸",
    searchKeywords: ["시카고", "chicago", "ㅅㅋㄱ", "피자", "건축"],
  },
  {
    id: "us-boston",
    name: "보스턴",
    englishName: "Boston",
    country: "미국",
    countryCode: "US",
    flag: "🇺🇸",
    searchKeywords: ["보스턴", "boston", "ㅂㅅㅌ", "하버드", "랍스터"],
  },
  {
    id: "us-washington",
    name: "워싱턴 D.C.",
    englishName: "Washington D.C.",
    country: "미국",
    countryCode: "US",
    flag: "🇺🇸",
    searchKeywords: ["워싱턴", "washington", "ㅇㅅㅌ", "백악관", "국회의사당"],
  },

  // 캐나다
  {
    id: "ca-vancouver",
    name: "밴쿠버",
    englishName: "Vancouver",
    country: "캐나다",
    countryCode: "CA",
    flag: "🇨🇦",
    searchKeywords: ["밴쿠버", "vancouver", "ㅂㅋㅂ", "스탠리파크", "개스타운"],
  },
  {
    id: "ca-toronto",
    name: "토론토",
    englishName: "Toronto",
    country: "캐나다",
    countryCode: "CA",
    flag: "🇨🇦",
    searchKeywords: ["토론토", "toronto", "ㅌㄹㅌ", "CN타워", "온타리오"],
  },
  {
    id: "ca-quebec",
    name: "퀘벡 시티",
    englishName: "Quebec City",
    country: "캐나다",
    countryCode: "CA",
    flag: "🇨🇦",
    searchKeywords: [
      "퀘벡",
      "quebec",
      "ㅋㅂ",
      "도깨비",
      "샤토프롱트낙",
      "퀘벡시티",
    ],
  },
  {
    id: "ca-montreal",
    name: "몬트리올",
    englishName: "Montreal",
    country: "캐나다",
    countryCode: "CA",
    flag: "🇨🇦",
    searchKeywords: [
      "몬트리올",
      "montreal",
      "ㅁㅌㄹㅇ",
      "노트르담",
      "지하도시",
    ],
  },
  {
    id: "ca-ottawa",
    name: "오타와",
    englishName: "Ottawa",
    country: "캐나다",
    countryCode: "CA",
    flag: "🇨🇦",
    searchKeywords: ["오타와", "ottawa", "ㅇㅌㅇ", "국회의사당", "리도운하"],
  },
  {
    id: "ca-banff",
    name: "밴프",
    englishName: "Banff",
    country: "캐나다",
    countryCode: "CA",
    flag: "🇨🇦",
    searchKeywords: ["밴프", "banff", "ㅂㅍ", "로키산맥", "레이크루이스"],
  },
  {
    id: "ca-whistler",
    name: "휘슬러",
    englishName: "Whistler",
    country: "캐나다",
    countryCode: "CA",
    flag: "🇨🇦",
    searchKeywords: ["휘슬러", "whistler", "ㅎㅅㄹ", "스키", "리조트"],
  },
  {
    id: "ca-calgary",
    name: "캘거리",
    englishName: "Calgary",
    country: "캐나다",
    countryCode: "CA",
    flag: "🇨🇦",
    searchKeywords: ["캘거리", "calgary", "ㅋㄱㄹ", "스탬피드"],
  },
  {
    id: "ca-victoria",
    name: "빅토리아",
    englishName: "Victoria",
    country: "캐나다",
    countryCode: "CA",
    flag: "🇨🇦",
    searchKeywords: [
      "빅토리아",
      "victoria",
      "ㅂㅌㄹㅇ",
      "부차트가든",
      "이너하버",
    ],
  },
  {
    id: "ca-niagara",
    name: "나이아가라 폴스",
    englishName: "Niagara Falls",
    country: "캐나다",
    countryCode: "CA",
    flag: "🇨🇦",
    searchKeywords: [
      "나이아가라",
      "niagara",
      "ㄴㅇㅇㄱㄹ",
      "폭포",
      "나이아가라폭포",
    ],
  },

  // === 국가 자체 ===
  {
    id: "kr-all",
    name: "대한민국",
    englishName: "South Korea",
    country: "대한민국",
    countryCode: "KR",
    flag: "🇰🇷",
    searchKeywords: ["대한민국", "한국", "korea", "kr", "ㄷㅎㅁㄱ"],
  },
  {
    id: "th-all",
    name: "태국",
    englishName: "Thailand",
    country: "태국",
    countryCode: "TH",
    flag: "🇹🇭",
    searchKeywords: ["태국", "thailand", "th", "ㅌㄱ"],
  },
  {
    id: "sg-all",
    name: "싱가포르",
    englishName: "Singapore",
    country: "싱가포르",
    countryCode: "SG",
    flag: "🇸🇬",
    searchKeywords: ["싱가포르", "singapore", "sg", "ㅅㄱㅍㄹ"],
  },
  {
    id: "mv-all",
    name: "몰디브",
    englishName: "Maldives",
    country: "몰디브",
    countryCode: "MV",
    flag: "🇲🇻",
    searchKeywords: ["몰디브", "maldives", "mv", "ㅁㄷㅂ"],
  },
  {
    id: "ph-all",
    name: "필리핀",
    englishName: "Philippines",
    country: "필리핀",
    countryCode: "PH",
    flag: "🇵🇭",
    searchKeywords: ["필리핀", "philippines", "ph", "ㅍㄹㅍ"],
  },
  {
    id: "vn-all",
    name: "베트남",
    englishName: "Vietnam",
    country: "베트남",
    countryCode: "VN",
    flag: "🇻🇳",
    searchKeywords: ["베트남", "vietnam", "vn", "ㅂㅌㄴ"],
  },
  {
    id: "kh-all",
    name: "캄보디아",
    englishName: "Cambodia",
    country: "캄보디아",
    countryCode: "KH",
    flag: "🇰🇭",
    searchKeywords: ["캄보디아", "cambodia", "kh", "ㅋㅂㄷㅇ"],
  },
  {
    id: "my-all",
    name: "말레이시아",
    englishName: "Malaysia",
    country: "말레이시아",
    countryCode: "MY",
    flag: "🇲🇾",
    searchKeywords: ["말레이시아", "malaysia", "my", "ㅁㄹㅇㅅㅇ"],
  },
  {
    id: "id-all",
    name: "인도네시아",
    englishName: "Indonesia",
    country: "인도네시아",
    countryCode: "ID",
    flag: "🇮🇩",
    searchKeywords: ["인도네시아", "indonesia", "id", "ㅇㄷㄴㅅㅇ"],
  },
  {
    id: "la-all",
    name: "라오스",
    englishName: "Laos",
    country: "라오스",
    countryCode: "LA",
    flag: "🇱🇦",
    searchKeywords: ["라오스", "laos", "la", "ㄹㅇㅅ"],
  },
  {
    id: "tw-all",
    name: "대만",
    englishName: "Taiwan",
    country: "대만",
    countryCode: "TW",
    flag: "🇹🇼",
    searchKeywords: ["대만", "taiwan", "tw", "ㄷㅁ"],
  },
  {
    id: "in-all",
    name: "인도",
    englishName: "India",
    country: "인도",
    countryCode: "IN",
    flag: "🇮🇳",
    searchKeywords: ["인도", "india", "in", "ㅇㄷ"],
  },
  {
    id: "np-all",
    name: "네팔",
    englishName: "Nepal",
    country: "네팔",
    countryCode: "NP",
    flag: "🇳🇵",
    searchKeywords: ["네팔", "nepal", "np", "ㄴㅍ"],
  },
  {
    id: "lk-all",
    name: "스리랑카",
    englishName: "Sri Lanka",
    country: "스리랑카",
    countryCode: "LK",
    flag: "🇱🇰",
    searchKeywords: ["스리랑카", "srilanka", "lk", "ㅅㄹㄹㅋ"],
  },
  {
    id: "cn-all",
    name: "중국",
    englishName: "China",
    country: "중국",
    countryCode: "CN",
    flag: "🇨🇳",
    searchKeywords: ["중국", "china", "cn", "ㅈㄱ"],
  },
  {
    id: "jp-all",
    name: "일본",
    englishName: "Japan",
    country: "일본",
    countryCode: "JP",
    flag: "🇯🇵",
    searchKeywords: ["일본", "japan", "jp", "ㅇㅂ"],
  },
  {
    id: "hr-all",
    name: "크로아티아",
    englishName: "Croatia",
    country: "크로아티아",
    countryCode: "HR",
    flag: "🇭🇷",
    searchKeywords: ["크로아티아", "croatia", "hr", "ㅋㄹㅇㅌㅇ"],
  },
  {
    id: "si-all",
    name: "슬로베니아",
    englishName: "Slovenia",
    country: "슬로베니아",
    countryCode: "SI",
    flag: "🇸🇮",
    searchKeywords: ["슬로베니아", "slovenia", "si", "ㅅㄹㅂㄴㅇ"],
  },
  {
    id: "es-all",
    name: "스페인",
    englishName: "Spain",
    country: "스페인",
    countryCode: "ES",
    flag: "🇪🇸",
    searchKeywords: ["스페인", "spain", "es", "ㅅㅍㅇ"],
  },
  {
    id: "pt-all",
    name: "포르투갈",
    englishName: "Portugal",
    country: "포르투갈",
    countryCode: "PT",
    flag: "🇵🇹",
    searchKeywords: ["포르투갈", "portugal", "pt", "ㅍㄹㅌㄱ"],
  },
  {
    id: "ma-all",
    name: "모로코",
    englishName: "Morocco",
    country: "모로코",
    countryCode: "MA",
    flag: "🇲🇦",
    searchKeywords: ["모로코", "morocco", "ma", "ㅁㄹㅋ"],
  },
  {
    id: "tr-all",
    name: "터키",
    englishName: "Turkey",
    country: "터키",
    countryCode: "TR",
    flag: "🇹🇷",
    searchKeywords: ["터키", "turkey", "tr", "ㅌㅋ"],
  },
  {
    id: "gr-all",
    name: "그리스",
    englishName: "Greece",
    country: "그리스",
    countryCode: "GR",
    flag: "🇬🇷",
    searchKeywords: ["그리스", "greece", "gr", "ㄱㄹㅅ"],
  },
  {
    id: "eg-all",
    name: "이집트",
    englishName: "Egypt",
    country: "이집트",
    countryCode: "EG",
    flag: "🇪🇬",
    searchKeywords: ["이집트", "egypt", "eg", "ㅇㅈㅌ"],
  },
  {
    id: "ge-all",
    name: "조지아",
    englishName: "Georgia",
    country: "조지아",
    countryCode: "GE",
    flag: "🇬🇪",
    searchKeywords: ["조지아", "georgia", "ge", "ㅈㅈㅇ"],
  },
  {
    id: "az-all",
    name: "아제르바이잔",
    englishName: "Azerbaijan",
    country: "아제르바이잔",
    countryCode: "AZ",
    flag: "🇦🇿",
    searchKeywords: ["아제르바이잔", "azerbaijan", "az", "ㅇㅈㄹㅂㅇㅈ"],
  },
  {
    id: "am-all",
    name: "아르메니아",
    englishName: "Armenia",
    country: "아르메니아",
    countryCode: "AM",
    flag: "🇦🇲",
    searchKeywords: ["아르메니아", "armenia", "am", "ㅇㄹㅁㄴㅇ"],
  },
  {
    id: "mn-all",
    name: "몽골",
    englishName: "Mongolia",
    country: "몽골",
    countryCode: "MN",
    flag: "🇲🇳",
    searchKeywords: ["몽골", "mongolia", "mn", "ㅁㄱ"],
  },
  {
    id: "imn-all",
    name: "내몽골",
    englishName: "Inner Mongolia",
    country: "내몽골",
    countryCode: "IMN",
    flag: "🇨🇳",
    searchKeywords: ["내몽골", "innermongolia", "imn", "ㄴㅁㄱ"],
  },
  {
    id: "hk-all",
    name: "홍콩",
    englishName: "Hong Kong",
    country: "홍콩",
    countryCode: "HK",
    flag: "🇭🇰",
    searchKeywords: ["홍콩", "hongkong", "hk", "ㅎㅋ"],
  },
  {
    id: "mo-all",
    name: "마카오",
    englishName: "Macau",
    country: "마카오",
    countryCode: "MO",
    flag: "🇲🇴",
    searchKeywords: ["마카오", "macau", "mo", "ㅁㅋㅇ"],
  },
  {
    id: "uz-all",
    name: "우즈베키스탄",
    englishName: "Uzbekistan",
    country: "우즈베키스탄",
    countryCode: "UZ",
    flag: "🇺🇿",
    searchKeywords: ["우즈베키스탄", "uzbekistan", "uz", "ㅇㅈㅂㅋㅅㅌ"],
  },
  {
    id: "kz-all",
    name: "카자흐스탄",
    englishName: "Kazakhstan",
    country: "카자흐스탄",
    countryCode: "KZ",
    flag: "🇰🇿",
    searchKeywords: ["카자흐스탄", "kazakhstan", "kz", "ㅋㅈㅎㅅㅌ"],
  },
  {
    id: "kg-all",
    name: "키르기스스탄",
    englishName: "Kyrgyzstan",
    country: "키르기스스탄",
    countryCode: "KG",
    flag: "🇰🇬",
    searchKeywords: ["키르기스스탄", "kyrgyzstan", "kg", "ㅋㄹㄱㅅㅅㅌ"],
  },
  {
    id: "au-all",
    name: "호주",
    englishName: "Australia",
    country: "호주",
    countryCode: "AU",
    flag: "🇦🇺",
    searchKeywords: ["호주", "australia", "au", "ㅎㅈ"],
  },
  {
    id: "nz-all",
    name: "뉴질랜드",
    englishName: "New Zealand",
    country: "뉴질랜드",
    countryCode: "NZ",
    flag: "🇳🇿",
    searchKeywords: ["뉴질랜드", "newzealand", "nz", "ㄴㅈㄹㄷ"],
  },
  {
    id: "us-all",
    name: "미국",
    englishName: "United States",
    country: "미국",
    countryCode: "US",
    flag: "🇺🇸",
    searchKeywords: ["미국", "unitedstates", "us", "ㅁㄱ"],
  },
  {
    id: "ca-all",
    name: "캐나다",
    englishName: "Canada",
    country: "캐나다",
    countryCode: "CA",
    flag: "🇨🇦",
    searchKeywords: ["캐나다", "canada", "ca", "ㅋㄴㄷ"],
  },
];

export function getRegionById(id: string): Region | undefined {
  return regionsList.find((region) => region.id === id);
}

export function getCountryById(id: string): Country | undefined {
  return countries.find((country) => country.id === id);
}

export const regions = {
  대한민국: regionsList
    .filter((r) => r.countryCode === "KR")
    .map((r) => r.name),
  태국: regionsList.filter((r) => r.countryCode === "TH").map((r) => r.name),
  싱가포르: regionsList
    .filter((r) => r.countryCode === "SG")
    .map((r) => r.name),
  몰디브: regionsList.filter((r) => r.countryCode === "MV").map((r) => r.name),
  필리핀: regionsList.filter((r) => r.countryCode === "PH").map((r) => r.name),
  베트남: regionsList.filter((r) => r.countryCode === "VN").map((r) => r.name),
  캄보디아: regionsList
    .filter((r) => r.countryCode === "KH")
    .map((r) => r.name),
  말레이시아: regionsList
    .filter((r) => r.countryCode === "MY")
    .map((r) => r.name),
  인도네시아: regionsList
    .filter((r) => r.countryCode === "ID")
    .map((r) => r.name),
  라오스: regionsList.filter((r) => r.countryCode === "LA").map((r) => r.name),
  대만: regionsList.filter((r) => r.countryCode === "TW").map((r) => r.name),
  인도: regionsList.filter((r) => r.countryCode === "IN").map((r) => r.name),
  네팔: regionsList.filter((r) => r.countryCode === "NP").map((r) => r.name),
  스리랑카: regionsList
    .filter((r) => r.countryCode === "LK")
    .map((r) => r.name),
  중국: regionsList.filter((r) => r.countryCode === "CN").map((r) => r.name),
  일본: regionsList.filter((r) => r.countryCode === "JP").map((r) => r.name),
  서유럽: regionsList.filter((r) => r.countryCode === "WEU").map((r) => r.name),
  동유럽: regionsList.filter((r) => r.countryCode === "EEU").map((r) => r.name),
  크로아티아: regionsList
    .filter((r) => r.countryCode === "HR")
    .map((r) => r.name),
  슬로베니아: regionsList
    .filter((r) => r.countryCode === "SI")
    .map((r) => r.name),
  모로코: regionsList.filter((r) => r.countryCode === "MA").map((r) => r.name),
  터키: regionsList.filter((r) => r.countryCode === "TR").map((r) => r.name),
  그리스: regionsList.filter((r) => r.countryCode === "GR").map((r) => r.name),
  이집트: regionsList.filter((r) => r.countryCode === "EG").map((r) => r.name),
  조지아: regionsList.filter((r) => r.countryCode === "GE").map((r) => r.name),
  아제르바이잔: regionsList
    .filter((r) => r.countryCode === "AZ")
    .map((r) => r.name),
  아르메니아: regionsList
    .filter((r) => r.countryCode === "AM")
    .map((r) => r.name),
  중동: regionsList.filter((r) => r.countryCode === "ME").map((r) => r.name),
  북유럽: regionsList.filter((r) => r.countryCode === "NEU").map((r) => r.name),
  아프리카: regionsList
    .filter((r) => r.countryCode === "AF")
    .map((r) => r.name),
  몽골: regionsList.filter((r) => r.countryCode === "MN").map((r) => r.name),
  내몽골: regionsList.filter((r) => r.countryCode === "IMN").map((r) => r.name),
  홍콩: regionsList.filter((r) => r.countryCode === "HK").map((r) => r.name),
  마카오: regionsList.filter((r) => r.countryCode === "MO").map((r) => r.name),
  우즈베키스탄: regionsList
    .filter((r) => r.countryCode === "UZ")
    .map((r) => r.name),
  카자흐스탄: regionsList
    .filter((r) => r.countryCode === "KZ")
    .map((r) => r.name),
  키르기스스탄: regionsList
    .filter((r) => r.countryCode === "KG")
    .map((r) => r.name),
  호주: regionsList.filter((r) => r.countryCode === "AU").map((r) => r.name),
  뉴질랜드: regionsList
    .filter((r) => r.countryCode === "NZ")
    .map((r) => r.name),
  미국: regionsList.filter((r) => r.countryCode === "US").map((r) => r.name),
  캐나다: regionsList.filter((r) => r.countryCode === "CA").map((r) => r.name),
  스페인: regionsList.filter((r) => r.countryCode === "ES").map((r) => r.name),
  포르투갈: regionsList
    .filter((r) => r.countryCode === "PT")
    .map((r) => r.name),
};
